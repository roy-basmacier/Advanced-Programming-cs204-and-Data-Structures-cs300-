...
HashTable<K, V>::HashTable(int s, double lf = 0.15, tableNode<K, V>* head = nullptr): size(s),loadFactor(lf), arr(new tableNode<K, V>*[s]())
{
	...
	//cout << "After preprocessing, the unique word count is " << clf*size << ". Current load ratio is " << clf << endl;
}
...
void HashTable<K, V>::rehash(tableNode<K, V>* head)
{
	...
	cout << "rehashing...\nprevious table size: " << nextPrime(2 * size);
	...
	size = newSize;
	cout << ", new table size: " << nextPrime(2 * size) << " current unique word count ";
	...
	while(itr)
	{
		...
		if(clf > loadFactor)
		{
			cout << uniqueWord << ", current load factor: " << uniqueWord/(nextPrime(2 * size)* 1.0) << endl;
			break;
		}
